...
class MyOrderDetailsProvider extends ChangeNotifier {
  ...
  Future<SuccessModel?> updateBookingPaymentStatus(context,{bookingId}) async {
    ...
    var request = http.Request(
        'POST',
        Uri.parse(
            'https://phpstack-1215628-4317594.cloudwaysapps.com/api/update/service-bookings/payemnt-status'));
  ...
  Future<SuccessModel?> updateOrderPaymentStatus(context,{orderId}) async {
    ...
    var request = http.Request(
        'POST',
        Uri.parse(
            'https://phpstack-1215628-4317594.cloudwaysapps.com/api/update/payment/status'));
    request.body = json.encode({
      "order_id": "$orderId"
    });
    request.headers.addAll(headers);
